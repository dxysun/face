...
//	var Wh = window.innerHeight;
	...
	/**************获取游戏图片并缓存*************/
	// 得到飞机图片
	this.plane_img = this.getImg('/static/plane/image/myplane1.png');
	// 得到背景图片
	this.bg_img = [this.getImg('/static/plane/image/bg_01.jpg'),
		this.getImg('/static/plane/image/bg_02.jpg'),
		this.getImg('/static/plane/image/bg_03.jpg'),
		this.getImg('/static/plane/image/bg_04.jpg'),
		this.getImg('/static/plane/image/bg_05.jpg')];
	// 得到敌机图片
	this.enemy_img = [this.getImg('/static/plane/image/dj10.png'),
		this.getImg('/static/plane/image/dj6.png'),
		this.getImg('/static/plane/image/dj1.png')];
	// 得到我军子弹图片
	this.bullet_img1 = this.getImg('/static/plane/image/bullet1.png');
	// 得到爆炸图片
	this.bomb_img = [this.getImg('/static/plane/image/b11.gif'),
		this.getImg('/static/plane/image/b10.gif'),
		this.getImg('/static/plane/image/b9.gif'),
		this.getImg('/static/plane/image/b8.gif'),
		this.getImg('/static/plane/image/b7.gif'),
		this.getImg('/static/plane/image/b6.gif'),
		this.getImg('/static/plane/image/b5.gif'),
		this.getImg('/static/plane/image/b4.gif'),
		this.getImg('/static/plane/image/b3.gif'),
		this.getImg('/static/plane/image/b2.gif'),
		this.getImg('/static/plane/image/b1.gif')];
	// 得到HP图片
	this.HP_img4 = this.getImg('/static/plane/image/img_HP.png');
	// 得到数字图片
	this.num_img = [this.getImg('/static/plane/image/0.png'),
		this.getImg('/static/plane/image/1.png'),
		this.getImg('/static/plane/image/2.png'),
		this.getImg('/static/plane/image/3.png'),
		this.getImg('/static/plane/image/4.png'),
		this.getImg('/static/plane/image/5.png'),
		this.getImg('/static/plane/image/6.png'),
		this.getImg('/static/plane/image/7.png'),
		this.getImg('/static/plane/image/8.png'),
		this.getImg('/static/plane/image/9.png')];
		
	/**************获取游戏音频并缓存*************/
	this.btToEp_audio = this.getAudio('/static/plane/audio/zd.mp3');//子弹打击敌机声音
	...
	this.ep_score = [100,300,500];//被消灭后得到的分数
...
GameStart.prototype = extend(Frame.prototype,{
	...
	/*	o.canvas.addEventListener('mousemove',function(event){
	    ...
	    document.addEventListener('facetrackingEvent',function (event) {
          //    console.log("position  x: " + event.x + "  y:" + event.y + "  width: " + event.width + " height: " + event.height);
           //   o.myPlane.mouseMove( event.x * 2.5, event.y * 3);
			  o.myPlane.mouseMove((230 - event.x) * 7, event.y * 4);
              ctx.strokeRect(event.x - 45,event.y - 45,event.width,event.height);
             }
         );
	/*    o.canvas.addEventListener('facetrackingEvent',function(event){
			var e = event || window.event;
			console.log("position  x: " + event.x + "  y:" + event.y + "  width: " + event.width + " height: " + event.height);
			o.myPlane.mouseMove(e.x,e.y);
		});*/
		//监听手指移动
		...
		function animate(thisTime,fps) {
		    if(!o.isPause){
		    	Tools.requestAnimFrame(animate,o.isPause);
		    }
		    o.draw(thisTime,fps);
		}
	},
	...
	gaming:function(thisTime,fps){
		...
		if(thisTime-this.bt_time>this.bt_intterval){
			...
			if(this.eb_speed % this.eb_level == 0)
			{
			    this.creatEnemyBullet();
			    if(this.eb_speed == 10000)
			        this.eb_speed = 1;
			}
			this.eb_speed ++;
			this.bt_time=thisTime;
		...
		this.bt_set.forEach(function(item){
			item.draw(fps,false);
			//碰撞检测
			item.checkToEpBump(false);
		});
		this.eb_set.forEach(function(item){
			item.draw(fps,true);
			//碰撞检测
			item.checkToEpBump(true);
		});
	...
	over:function(){
        ...
        $('#canvas').drawText({
                ...
                width: 200,
                height: 200,
                x: o.cvWidth/2, y: o.cvHeight/2,
                fontSize: 55,
                fontFamily: 'Verdana, sans-serif',
                text: 'Game Over',

            });
        //    send_data(o.score_int);
        if(!o.isPause)
        {
            $.post("/face/record_score/",{
			            score: o.score_int
		            },
	...
	//	console.log("score " + o.score_int )
	    $('#canvas').drawText({
                ...
                width: 28,
                height: 38,
                x: o.cvWidth-300, y: y + 20,
                fontSize: 40,
                fontFamily: 'Verdana, sans-serif',
                text: 'LEVEL ' + o.hard_level,

            });
        if(o.score_int > 5000 && o.hard_level < 2)
        {
            o.ep_intterval[0] = 2000;
            o.ep_intterval[1] = 5000;
            o.ep_intterval[2] = 12000;
            console.log("score more hard 2 " + o.score_int );
            o.eb_level = 10;
            o.hard_level = 2;
            $('#canvas').drawText({
                fillStyle: '#FF0000',
        ...
        if(o.score_int > 10000 && o.hard_level < 3)
        {
            o.ep_intterval[0] = 1000;
            o.ep_intterval[1] = 2000;
            o.ep_intterval[2] = 6000;
            console.log("score more hard 3" );
            o.eb_level = 5;
            o.hard_level = 3;
            $('#canvas').drawText({
                fillStyle: '#FF0000',
	...
	//生成敌机子弹
	creatEnemyBullet:function(){
	    var game = this;
	    this.ep_set.forEach(function(item){
			var mp = item;
		    var img = game.bullet_img1;
		    var w = game.bt_w;
		    var h = game.bt_h;
		    var x = mp.x_int+(mp.w_int-w)/2;
		    var y = mp.y_int + mp.h_int -h;
		    var vx = game.bt_vx;
		    var vy = game.bt_vy;
		    var hurt = game.bt_hurt;
		    var bt = new Bullet(img,x,y,w,h,vx,vy,hurt,true,game);
		    game.eb_set.add(bt);
		});
	},
	//创造敌机
